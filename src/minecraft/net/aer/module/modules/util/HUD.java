    ...
    public void onDraw2D(EventRender2D event) {
		...
		if (direction.getObject()) {
			if (coords.getObject() || netherCoords.getObject()) {
                RenderUtils2D.drawString(Fonts.big, PlayerUtil.direction3(Aer.minecraft.player), 5f, (float) scaledRes.getScaledHeight() - 18 - 15, col, true);
            } else {
                RenderUtils2D.drawString(Fonts.big, PlayerUtil.direction3(Aer.minecraft.player), 5f, (float) scaledRes.getScaledHeight() - 18, col, true);
            }
		}
		...
		if (armor.getObject()) {
			...
			for (ItemStack is : minecraft.player.inventory.armorInventory) {
                RenderUtils2D.drawItem(is, x - offset, y, 1f, Fonts.big);
